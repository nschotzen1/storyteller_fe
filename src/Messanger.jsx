  ...
  return (
    ...
    >
      ...
      <img
        src={avatar}
        alt={sender}
        className="w-8 h-8 rounded-full border border-yellow-700 shadow-sm"
      />

      <div className="flex flex-col max-w-[80%]">
        {/* Sender identity badge (for assistant only) */}
        {sender === 'system' && (
          <div className="mb-1 text-xs text-yellow-400 font-fell leading-snug tracking-wide group relative">
            <div className="flex items-center gap-1 font-semibold">
              ...
              {/* Wax seal emoji */}
              <span className="ml-1 wax-seal text-red-700 transition-transform duration-500 ease-in-out group-hover:rotate-[15deg] group-hover:scale-110 drop-shadow">
                🕯️
              </span>
            </div>
...
const sendMessageToAPI = async (text) => {
  try {
    ...
    return data; // { reply, has_chat_ended }
  } catch (err) {
  ...
  const sendMessage = async () => {
    ...
    setChatEnded(has_chat_ended);
    {chatEnded && (
      <div className="text-yellow-600 text-xs text-center mt-2 opacity-60 italic font-fell">
