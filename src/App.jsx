...
import BrewingGame from './brewing/BrewingGame';

// Rename original App to StorytellerGame
function StorytellerGame() {
  ...
  return (
    <div className="w-full h-full bg-black flex items-center justify-center">
      <div
        className={`relative h-[720px] bg-black isolate overflow-hidden transition-all duration-[3000ms] ease-in-out ${curtainShouldExpand ? 'w-full max-w-screen-xl' : 'w-[360px]'
          }`}
      >
...
// Main App Switcher
function App() {
  const [view, setView] = useState('brewing'); // 'brewing' or 'storyteller'
  ...
  if (view === 'brewing') {
    return (
      <>
        <BrewingGame />
        {/* Simple Switcher UI */}
        <div className="fixed bottom-2 right-2 opacity-20 hover:opacity-100 transition-opacity z-[9999]">
          <button onClick={() => setView('storyteller')} className="text-xs bg-black text-white px-2 py-1 rounded">
  ...
  return (
    <>
      <StorytellerGame />
      <div className="fixed bottom-2 right-2 opacity-20 hover:opacity-100 transition-opacity z-[9999]">
        <button onClick={() => setView('brewing')} className="text-xs bg-white text-black px-2 py-1 rounded">
          Switch to Brewing
        </button>
      </div>
    </>
